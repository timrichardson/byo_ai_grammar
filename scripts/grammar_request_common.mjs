...
const BASE_SYSTEM_PROMPT = [
  ...
  "Ignore spelling mistakes, apart from homophone confusions and function-word confusions that cause grammatical errors.",
  ...
  "When correcting a homophone or function-word confusion, prefer the matching word from the same confusion set before considering any other rewrite.",
...
function getNeedsChangeField(record) {
...
export function normalizeCorrectedTextResponse(raw, sourceText) {
  ...
  const correctedText = needsChangeField?.value === false ? sourceText : correctedTextField.value;
...
export function analyzeGrammarResponse({ responseText, activeText }) {
  ...
  if (isRecord(content)) {
    ...
    return {
      ...
      needsChange: normalized.needsChange
    };
  }
